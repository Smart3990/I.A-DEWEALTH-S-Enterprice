...
function run() {
  ...
  // Determine where static assets are: prefer fresh .output/public from Vite build
  const sourceStaticDir = fs.existsSync(outputPublicDir)
    ? outputPublicDir
  ...
  // If source was .output/public, clean stale dist and sync fresh output to dist root first
