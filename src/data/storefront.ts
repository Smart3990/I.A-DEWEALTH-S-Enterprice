...
import { getAllProducts, isLegacyOldProduct, getCombinedCatalogProducts } from "./products-store";
...
export async function fetchStorefront(): Promise<Storefront> {
  try {
    ...
    // If client direct Supabase query returned no products, fall back to server API endpoint
    if (rawProdRows.length === 0) {
      try {
        ...
        console.warn("[Storefront] Server API product fallback bypassed:", e);
      }
    }
    ...
    const finalCategories = mappedCats.length ? mappedCats : bundledCategories;
    const finalProducts = Array.from(prodsMap.values());
    applyCatalogSnapshot(finalCategories, finalProducts);
