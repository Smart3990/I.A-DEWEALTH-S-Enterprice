...
import {
  carCategories,
  carsInCategory as seedCarsInCategory,
  waCarLink,
  WHATSAPP,
  type Car,
} from "@/data/products";
...
export const Route = createFileRoute("/cars/$category")({
  loader: ({ params }) => {
    ...
    return { category, list: seedCarsInCategory(category.slug) };
  },
  head: ({ loaderData }) => {
    if (!loaderData) {
      return {
...
function CategoryPage() {
  ...
  const allCars = useMemo(() => {
    ...
    // Dynamic database vehicles
    vehicleProducts.forEach((p) => {
      carMap.set(p.id, catalogProductToCar(p));
    });

    // Seed vehicles
    seedCarsInCategory(category.slug).forEach((c) => {
      if (!carMap.has(c.id)) {
        carMap.set(c.id, c);
      }
    });
  ...
  const list = useMemo(() => {
    return allCars.filter((car) => category.bodyTypes.includes(car.bodyType));
  }, [allCars, category.bodyTypes]);

  const carsInCategory = (slug: string) => {
    const cat = carCategories.find((c) => c.slug === slug);
    if (!cat) return [];
    return allCars.filter((car) => cat.bodyTypes.includes(car.bodyType));
  };
    ...
    <div className="min-h-screen bg-background font-body text-foreground">
      ...
      <nav className="border-b border-border bg-card">
        <div className="mx-auto flex max-w-container-max gap-2 overflow-x-auto px-4 py-3 lg:px-6">
          {carCategories
            .filter((c) => carsInCategory(c.slug).length > 0)
            .map((c) => (
              <Link
                key={c.slug}
                to="/cars/$category"
