...
const groups: { id: string; title: string; fields: Field[] }[] = [
  ...
  },
  {
    ...
    fields: [
      { key: "whatsapp_number", label: "WhatsApp number (e.g. 233241118229 or 0241118229)" },
  ...
  {
    id: "superdeals",
    title: "SuperDeals Flash Countdown Settings",
    fields: [
      { key: "superdeals_hours", label: "Countdown Hours (e.g. 8)", type: "number" },
      { key: "superdeals_minutes", label: "Countdown Minutes (e.g. 29)", type: "number" },
      { key: "superdeals_seconds", label: "Countdown Seconds (e.g. 33)", type: "number" },
    ],
  },
];
...
function SettingsAdmin() {
  ...
  const [form, setForm] = useState<any>(() => {
    ...
    return {
      ...unified,
      id: "default",
      store_name: unified.storeName,
      store_description: unified.storeDescription,
      logo_url: unified.logoUrl,
      favicon_url: unified.faviconUrl,
      whatsapp_number: unified.whatsappNumber,
      whatsapp_message: unified.whatsappMessage,
      working_hours: unified.workingHours,
      x_url: unified.xUrl,
      footer_description: unified.footerDescription,
      copyright_text: unified.copyrightText,
      favorites_enabled: unified.favoritesEnabled,
      cart_enabled: unified.cartEnabled,
      max_qty_per_product: unified.maxQtyPerProduct,
      superdeals_hours: unified.superdealsHours ?? 8,
      superdeals_minutes: unified.superdealsMinutes ?? 29,
      superdeals_seconds: unified.superdealsSeconds ?? 33,
    };
  });
  ...
  const detectCurrentLocation = () => {
    ...
    setIsLocating(true);
    setLocatingStatus("Detecting your exact GPS coordinates…");

    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          const { latitude, longitude } = pos.coords;
          setLocatingStatus("Looking up address name…");

          let placeAddress = `${latitude.toFixed(6)}, ${longitude.toFixed(6)}`;
          try {
            ...
            if (resp.ok) {
              ...
              if (resData?.display_name) {
                placeAddress = resData.display_name;
              }
            }
          } catch (e) {
          ...
          setDeliveryConfig((prev) => ({
            ...
            mapLocation: placeAddress,
            mapEmbedUrl: mapUrl,
            visitUsLocation: placeAddress.split(",")[0] || placeAddress,
          }));
          ...
          setLocatingStatus(`Saved: ${placeAddress.slice(0, 40)}…`);
          toast.success("Business location updated to your current location!");
          setTimeout(() => setLocatingStatus(null), 5000);
        } catch (err) {
          console.error("Location handling error:", err);
          setLocatingStatus("Could not complete location lookup");
          setTimeout(() => setLocatingStatus(null), 3500);
        } finally {
          setIsLocating(false);
        }
      },
      (err) => {
        setIsLocating(false);
        setLocatingStatus(`GPS notice: ${err.message || "Permission not granted"}`);
        setTimeout(() => setLocatingStatus(null), 4000);
      },
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        console.warn("Notice: Server settings fetch error, trying direct fallback:", err);
      }
      try {
        const { data, error } = await supabase
          ...
          .maybeSingle();
        if (!error && data) {
          return { settings: data, hubConfig: {} };
        }
      } catch (err) {
        console.warn("Notice: Direct site settings fetch failed:", err);
      }
      return null;
    },
  });
  ...
  useEffect(() => {
    if (data && data.settings) {
      setForm((prev) => ({
        ...prev,
        ...data.settings,
        phone: data.settings.contact_phone || data.settings.phone || prev.phone,
        email: data.settings.contact_email || data.settings.email || prev.email,
        facebook: data.settings.facebook_url || data.settings.facebook || prev.facebook,
        instagram: data.settings.instagram_url || data.settings.instagram || prev.instagram,
        tiktok: data.settings.tiktok_url || data.settings.tiktok || prev.tiktok,
        xUrl: data.settings.x_url || data.settings.xUrl || prev.xUrl,
        x_url: data.settings.x_url || data.settings.xUrl || prev.x_url,
        youtube: data.settings.youtube_url || data.settings.youtube || prev.youtube,
      }));
    }
    if (data && data.hubConfig && Object.keys(data.hubConfig).length > 0) {
      setDeliveryConfig((prev) => ({
        ...prev,
        ...data.hubConfig,
      }));
      setForm((prev) => ({
        ...prev,
        working_hours: data.hubConfig.workingHours || prev.working_hours,
        workingHours: data.hubConfig.workingHours || prev.workingHours,
        logo_url: data.hubConfig.logoUrl || prev.logo_url,
        logoUrl: data.hubConfig.logoUrl || prev.logoUrl,
        favicon_url: data.hubConfig.faviconUrl || prev.favicon_url,
        faviconUrl: data.hubConfig.faviconUrl || prev.faviconUrl,
        store_description: data.hubConfig.storeDescription || prev.store_description,
        storeDescription: data.hubConfig.storeDescription || prev.storeDescription,
      }));
    }
  }, [data]);
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      // 2. Save site settings to unified reactive storage
      if (form) {
        saveUnifiedSettings({
          ...form,
          xUrl: form.x_url || form.xUrl,
          x_url: form.x_url || form.xUrl,
          phone: form.phone || deliveryConfig.phone,
          email: form.email || deliveryConfig.email,
          address: form.address || deliveryConfig.address,
          whatsapp_number: form.whatsapp_number || deliveryConfig.whatsappNumber,
          working_hours: deliveryConfig.workingHours,
        });
      }

      // 3. Post directly to server API to ensure cross-device database persistence
      try {
        const payload = {
          ...form,
          x_url: form.x_url || form.xUrl,
          ...
          deliveryConfig: {
            ...deliveryConfig,
            storeDescription: form.store_description || form.storeDescription || "",
            logoUrl: form.logo_url || form.logoUrl || "",
            faviconUrl: form.favicon_url || form.faviconUrl || "",
          },
        };
        ...
        if (!res.ok) {
          ...
          console.warn("API settings save warning:", errData);
        }
      } catch (err) {
        console.warn("Failed to post settings to API:", err);
      }
      ...
      >
        ...
        {(activeTab === "all" || activeTab === "delivery") && (
          <Panel>
            ...
            {/* Current Location GPS Quick Setter */}
            <div className="mb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-2xl bg-emerald-50/90 border border-emerald-200 p-3.5 shadow-2xs">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs font-bold text-slate-900">Business Default Location</div>
                  <div className="text-[11px] text-emerald-800">
                    {locatingStatus ||
                      "Quickly detect your exact device coordinates and set as business address"}
                  </div>
                </div>
              </div>
              <button
                ...
                disabled={isLocating}
                className="inline-flex cursor-pointer items-center justify-center gap-1.5 rounded-xl bg-emerald-600 px-3.5 py-2 text-xs font-bold text-white shadow-xs transition hover:bg-emerald-700 disabled:opacity-60 shrink-0"
              >
                <MapPin className="h-3.5 w-3.5" />
