...
import { cars as seedCars, carCategories, waCarLink, WHATSAPP, type Car } from "@/data/products";
import { useProducts } from "@/data/use-catalog";
import { type CatalogProduct, ancestorsOf } from "@/data/catalog";
...
function isVehicleProduct(p: CatalogProduct): boolean {
  if (!p) return false;
  ...
  return (
    ...
    cat === "suvs" ||
    cat === "motorbikes" ||
    cat === "commercial-vehicles" ||
    cat === "car-accessories" ||
    ancestorsOf(p.categoryId).some((c) => c.id === "cars-vehicles")
  );
}

function catalogProductToCar(p: CatalogProduct): Car {
  const brand = p.brand || p.name.split(" ")[0] || "Vehicle";
  const yearMatch = p.name.match(/\b(20\d\d|19\d\d)\b/);
  ...
  if (catLower.includes("suv") || nameLower.includes("suv") || nameLower.includes("4x4")) {
    bodyType = "SUV";
  } else if (
    ...
    bodyType = "Pickup";
  } else if (nameLower.includes("coupe")) {
    bodyType = "Coupe";
  } else if (nameLower.includes("electric") || nameLower.includes(" ev")) {
    bodyType = "Hatchback";
  }
  ...
  return {
    ...
    mileage: "Verified Inspection",
    color: "As Shown",
    price: p.price,
    image: p.image || (Array.isArray(p.images) ? p.images[0] : ""),
    featured: Boolean(p.featured || p.topPick || p.bestSeller),
  };
}
...
function CarsPage() {
  ...
  const cars = useMemo(() => {
    const vehicleProducts = allProducts.filter(isVehicleProduct);
    const carMap = new Map<string, Car>();

    // 1. Authoritative dynamic vehicles from database
    vehicleProducts.forEach((p) => {
      carMap.set(p.id, catalogProductToCar(p));
    });

    // 2. Showcase seed cars
    seedCars.forEach((c) => {
      if (!carMap.has(c.id)) {
        carMap.set(c.id, c);
      }
  ...
  const carsInCategory = (slug: string) => {
    const cat = carCategories.find((c) => c.slug === slug);
    if (!cat) return [];
    return cars.filter((car) => cat.bodyTypes.includes(car.bodyType));
  };
  ...
  return (
    <div className="min-h-screen bg-background font-body text-foreground">
      ...
      {/* Categories */}
      <section className="mx-auto max-w-container-max px-3 sm:px-4 pt-8 sm:pt-12 lg:px-6 lg:pt-16">
        <div className="mb-6 sm:mb-8">
          <p className="text-[11px] sm:text-xs font-bold uppercase tracking-[0.25em] text-primary">
            Browse By Category
          </p>
          <h2 className="mt-1 font-sans text-2xl sm:text-3xl font-black tracking-tight text-foreground lg:text-4xl">
            Car Categories
          </h2>
          <p className="mt-1.5 max-w-xl text-xs sm:text-sm text-muted-foreground">
        ...
        <div className="grid grid-cols-2 gap-2.5 sm:gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {carCategories
            .filter((cat) => carsInCategory(cat.slug).length > 0)
            .map((cat) => {
              ...
              return (
                <Link
                  ...
                  className="group relative overflow-hidden rounded-2xl sm:rounded-3xl border border-border shadow-xs transition duration-300 hover:-translate-y-1 hover:shadow-xl"
                >
                  <img
                    src={cat.image}
                    alt={cat.label}
                    loading="lazy"
                    className="h-36 sm:h-56 w-full object-cover transition duration-500 group-hover:scale-110"
                  />
                  <div
                  ...
                  <div className="absolute inset-0 flex flex-col justify-end p-3 sm:p-6">
                    <p
                      className={`text-[9px] sm:text-[11px] font-extrabold uppercase tracking-[0.2em] sm:tracking-[0.25em] ${cat.accent}`}
                    >
                      {cat.tagline}
                    </p>
                    <h3 className="mt-0.5 sm:mt-1 font-sans text-base sm:text-2xl font-black text-white leading-tight">
                      {cat.label}
                    </h3>
                    <p className="mt-0.5 text-[10px] sm:text-xs font-semibold text-white/80">
