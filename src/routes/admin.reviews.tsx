...
import { PageHead, Panel, Button, Badge } from "@/components/admin/kit";
...
function AdminReviewsPage() {
  const { all, pending, approved, rejected, approve, reject, remove } = useAllReviews();
  ...
  const handleDelete = (review: ReviewItem) => {
    if (confirm(`Permanently delete review from "${review.name}"?`)) {
      remove(review.id);
      logActivity(`Deleted review from "${review.name}"`);
